...
static const char* pipeline_state_to_string(PipelineState state) {
  switch (state) {
...
void ADFPipeline::check_for_pipeline_events_(){
  ...
  if (ret == ESP_OK) {
    ...
    if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.cmd == AEL_MSG_CMD_REPORT_STATUS){
      ...
      audio_element_handle_t el = static_cast<audio_element_handle_t>(msg.source);
...
void ADFPipeline::set_state_(PipelineState state) {
  esph_log_d(TAG, "State changed from %s to %s", 
             pipeline_state_to_string(this->state_),
...
bool ADFPipeline::stop_() { 
  return audio_pipeline_stop(adf_pipeline_) == ESP_OK; 
}
...
bool ADFPipeline::build_adf_pipeline_() {
  if (adf_pipeline_ != nullptr) {
    esph_log_d(TAG, "Pipeline was already built.");
    return true;
  }
  
  // Updated for ESP-IDF 5.1.5 compatibility
  audio_pipeline_cfg_t pipeline_cfg = AUDIO_PIPELINE_DEFAULT_CFG();
  pipeline_cfg.rb_size = 8 * 1024;
  
  adf_pipeline_ = audio_pipeline_init(&pipeline_cfg);
  if (adf_pipeline_ == NULL) {
    esph_log_e(TAG, "Failed to initialize audio pipeline");
    return false;
  }
  ...
  // Use smart pointer to ensure memory is freed
  std::unique_ptr<const char*> link_tag_ptrs(new const char*[tags_vector.size()]);
  for (size_t i = 0; i < tags_vector.size(); i++) {
    link_tag_ptrs.get()[i] = tags_vector[i].c_str();
    esph_log_d(TAG, "pipeline tag %zu, %s", i, link_tag_ptrs.get()[i]);
  }

  if (audio_pipeline_link(adf_pipeline_, link_tag_ptrs.get(), tags_vector.size()) != ESP_OK) {
  ...
  if (adf_pipeline_event_ == NULL) {
    esph_log_e(TAG, "Couldn't create pipeline event interface");
    return false;
  }
